...
public class TypedViolationModule1Test extends BaseTestForLibIndependentOfDeltaSpike {
    @Inject
    private Validator validator;

    @Test
    public void validate() {
        Set<ConstraintViolation<TestPerson1>> violations = validator.validate(new TestPerson1("g", "p"));
        Assert.assertThat(violations.size(), is(1));

        Assert.assertThat(violations.iterator().next().getMessage(), is("[1] The length of 'Surname' should be between 2 and " + Integer.MAX_VALUE));
    }
}
